...
function onCreate()
	makeAnimatedLuaSprite('fx', 'phantasmbg/vintage', 0, 0)
	...
	for i = 1,2 do
		makeLuaSprite(bgs[i], 'phantasmbg/'..bgs[i], 0, 0)
		addLuaSprite(bgs[i], false)
	end
	addCharacterToList('reialt_phantasm', 'boyfriend')
	precacheSound('stat')

	setPropertyFromClass('GameOverSubstate', 'characterName', 'rei_phantasm')
end
...
function fleetON()
	...
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'reialt_phantasm')
end
...
function fleetOFF()
	...
	doTweenAlpha('fleetON', 'fx', 0, 1, 'linear')
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'rei_phantasm')
end
